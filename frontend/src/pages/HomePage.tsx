...
const audiences = [
  { icon: <FiHome size={28} />, label: "Families", desc: "Home-style meals for busy households" },
  { icon: <FiUsers size={28} />, label: "Events & Community", desc: "Catering for celebrations" },
  { icon: <FiBriefcase size={28} />, label: "Corporate", desc: "Office meals & meetings" },
  { icon: <FiStar size={28} />, label: "Busy Professionals", desc: "Quick, quality meals" },
  { icon: <FiShield size={28} />, label: "Government Organisations", desc: "Institutional food services" },
];

const trustPoints = [
  { icon: <FiCheckCircle size={24} />, title: "Authentic Taste", desc: "Traditional recipes & flavours" },
  { icon: <FiCheckCircle size={24} />, title: "Consistent Quality", desc: "Every order, every time" },
  { icon: <FiCheckCircle size={24} />, title: "Hygienic Prep", desc: "Strict safety standards" },
  { icon: <FiCheckCircle size={24} />, title: "Reliable Service", desc: "On-time delivery guaranteed" },
  { icon: <FiCheckCircle size={24} />, title: "Professional Setup", desc: "Modern food operations" },
];
    ...
    <>
      ...
      <section className="py-16 bg-background">
        <div className="container mx-auto px-4 flex flex-col lg:flex-row gap-10 items-center">
          <motion.div initial={{ opacity: 0, x: -60 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="lg:w-1/2">
            <img src="/images/food-prep.jpg" alt="Food Preparation" className="rounded-2xl shadow-xl w-full h-96 object-cover" loading="lazy" />
          </motion.div>
      ...
      <section className="py-16 bg-muted">
        <div className="container mx-auto px-4">
          <motion.h2 initial={{ opacity: 0, y: -20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-4xl font-bold text-foreground text-center mb-12">
            Our Services
          </motion.h2>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="text-center mt-10">
            <Link to="/services" className="text-primary font-semibold text-lg hover:underline inline-flex items-center gap-2">
              View All Services →
            </Link>
          </motion.div>
        </div>
      ...
      <section className="py-16 bg-background">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: -20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            ...
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">Delivering authentic food solutions to diverse communities across Calgary</p>
          </motion.div>
          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            {audiences.map((a) => (
              <motion.div key={a.label} variants={item}
                ...
                <div className="text-primary group-hover:text-primary-foreground transition-colors duration-300 bg-background group-hover:bg-primary-foreground/20 p-4 rounded-full">{a.icon}</div>
                <div className="text-center">
                  <span className="font-bold text-base block mb-2">{a.label}</span>
      ...
      <section className="py-16 bg-muted">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: -20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            ...
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">Five reasons why Calgary trusts ABHYY'S FOOD for authentic ethnic cuisine</p>
          </motion.div>
          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            {trustPoints.map((point) => (
              <motion.div key={point.title} variants={item} className="flex flex-col items-center text-center gap-3 bg-background p-6 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-2 border border-transparent hover:border-primary">
                <div className="text-primary bg-primary/10 p-4 rounded-full">{point.icon}</div>
                <div>
      ...
      <section className="py-16 bg-background">
        <div className="container mx-auto px-4">
          <div className="max-w-5xl mx-auto bg-gradient-to-r from-primary/5 to-primary/10 rounded-3xl p-10 border border-primary/20">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="flex flex-col lg:flex-row items-center gap-8">
              ...
              <div className="lg:w-2/3 text-center lg:text-left">
                ...
                <p className="text-muted-foreground text-lg leading-relaxed mb-6">
                  We follow strict hygiene, preparation, and food safety standards to ensure every product meets quality and safety expectations. From ingredient sourcing to final delivery, our commitment to food safety guides every step of our operation.
                </p>
                <Link to="/food-safety" className="inline-flex items-center gap-2 text-primary font-semibold text-lg hover:underline">
